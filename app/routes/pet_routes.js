...
router.get('/pets', (req, res, next) => {
    ...
    // if we wanted to make this a protected resource, we'd just need to add that middlware as the second arg to our get(like we did in creat for our post)
    Pet.find()
        .populate('owner')
        .then(pets => {
            // pets will be an array of mongoose documents
            // so we want to turn them into POJO (plain ol' hs objects)
...
router.get('/pets/:id', (req, res, next) => {
    ...
    Pet.findById(req.params.id)
        // if you also want to populate the owner as an object
        .populate('owner')
        .then(handle404)
...
router.post('/pets', requireToken, (req, res, next) => {
    ...
    Pet.create(req.body.pet)
        .then(pet => {
            ...
            res.status(201).json({ pets: pet.toObject() })
        })
        // if an error occurs, send to to the error handler
        .catch(next)
})
